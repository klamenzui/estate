(function($) {
	...
	$.Tabledit = function(element, options) {
					...
					for (var i = 0; i < settings.columns.editable.length; i++) {
						...
						$td.each(function() {
							...
							if (typeof settings.columns.editable[i][2] !== 'undefined') {
								if(settings.columns.editable[i][2] == 'date') {
									var input = '<div class="input-group mb-2"><div class="input-group-prepend"><span class="input-group-text">📅</span></div><input  class="tabledit-input ' + settings.inputClass + 'form-control mr-sm-2 " type="date" placeholder="Date" aria-label="Date" name="' + settings.columns.editable[i][1] + '" value="' + $(this).text() + '" style="display: none;" disabled></div>';
								} else {
		...
		function ajax(action)
		{
			...
			var jqXHR = $.post($table.attr('url') || settings.url, serialize, function(data, textStatus, jqXHR) {
